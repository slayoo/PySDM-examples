...
from PySDM.dynamics import Freezing, AmbientThermodynamics, Condensation
from PySDM.environments import Parcel
from PySDM.initialisation import equilibrate_wet_radii
from PySDM.products import IceWaterContent, AmbientTemperature, ParcelDisplacement
...
class Simulation(BasicSimulation):
    def __init__(self, settings: Settings):
        ...
        ccn_diameter, ccn_conc_float = sampled_ccn_diameter_number_concentration_spectrum(
            size_range=settings.ccn_dry_diameter_range,
            n_sd=settings.ccn_sampling_n
        )
        ...
        attributes['volume'] = settings.formulae.trivia.volume(radius=equilibrate_wet_radii(
        ...
        for attribute, data in attributes.items():
            attributes[attribute] = np.concatenate((
                ...
                np.full(
                    ...
                    data[-1] if attribute != 'n' else data[-1] / settings.in_sampling_n
                )
            ))
        ...
        self.steps = int(settings.displacement / settings.vertical_velocity / settings.timestep)
